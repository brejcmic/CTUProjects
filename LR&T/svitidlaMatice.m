...
for generace = 1:1:pop.gen
    ...
    for clen = 1:1:pop.N
        %------------------------------------------------------------------
        %Vsechna svitidla tohoto clena populace vuci podlaze
        %------------------------------------------------------------------

        xs= pop.dna(clen, 1:2:((2*svt.N)-1))'*ones(1, podlaha.N);
        ys= pop.dna(clen, 2:2:(2*svt.N))'*ones(1, podlaha.N);
        zs= svt.z*ones(svt.N, podlaha.N);

        xb= ones(svt.N, 1)*podlaha.x;
