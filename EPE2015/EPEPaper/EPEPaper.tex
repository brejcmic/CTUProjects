...
Jan Zálešák\IEEEauthorrefmark{1}, 
Zuzana Panská\IEEEauthorrefmark{1} 
Marek Bálský\IEEEauthorrefmark{1}}

\IEEEauthorblockA{
\IEEEauthorrefmark{1}CTU in Prague, Faculty of Electrical Engineering, Department of Electrical Power Engineering\\
Praha 6, Technická 2, 166 27\\
...
\begin{abstract}
%\boldmath
The paper deals with lighting road design. The demands on illuminance are defined in standard~\v{C}SN~EN~13201 in case of Czech Republic. There are several parameters that can designer change to get the optimal solution fulfill the standard. The distance between pillars, their heights, the lamp overlap and tilt have to be defined. Many parameters make the optimization difficult. The Paper solves the optimization via genetic algorithm. The fitness function that is convergent to good solutions is vital point for this type of algorithms. The paper shows that the solutions found by the genetic algorithm fulfill the demands and it also shows the way, how the fitness function can be created.
...
\section{Conclusion}
Authors presented results of road lightening design using genetic algorithm in the paper. The results were compared with outputs of common used software DIALux. It was proved that found solutions satisfies all demands defined in standards.

Even the algorithm found the solutions in only 60 generations, the calculation last approximately 10~minutes. The reason was lot of calculated points ($8000+6000$) and relatively large population (200) in each generation. The directives recommend much thinner points concentrations and they do not deal with lighting out of the sidewalk. The algorithm lasted only 2~minutes in case of recommended count of points.

The genetic algorithm can help the designer to find good design even if he is limited by some constraints. These constraints are simply included to limitations of each searched parameter or are taken into account in the fitness function. The fact, that the lighting out of the sidewalk can be taken into account is also very helpful. 
...
\bibitem{Zelinka2009}
ZELINKA, Ivan, Zuzana OPLATKOVÁ, Miloš ŠEDA, Pavel OŠMERA a František VÈELAØ. \textit{Evoluèní výpoèetní techniky: principy a aplikace}. 1. èeské vyd. Praha: BEN, 2009, 534~s. ISBN 978-80-7300-218-3. 
