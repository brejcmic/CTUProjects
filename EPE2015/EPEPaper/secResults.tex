...
\begin{table}[htb]
	\renewcommand{\arraystretch}{1.3}
	\caption{DIALux calculated design illuminances in lux for one side lamp placement at the surface of the road (fig.~\ref{fig:lumIntDistr})}
...
According to standard~\cite{CSN_EN_13201-3} calculation points shall not be spaced in the longitudinal directions more than 3 m from each other, so 14 calculation points were used during calculations of DIALux instead of 10 depicted in figure~\ref{fig:lumIntDistr}.

\begin{table}[htb]
	\renewcommand{\arraystretch}{1.3}
	\caption{Calculated design illuminances in lux using values from tab.~\ref{tab:DialuxOneSideLamps})}
